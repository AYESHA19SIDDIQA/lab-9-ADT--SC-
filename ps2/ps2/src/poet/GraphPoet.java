...
public class GraphPoet {
    ...
    public String poem(String input) {
        // Preserve original words with punctuation and clean for graph analysis
        Map<String, String> wordMapping = new HashMap<>();
        String[] originalWords = input.split("\\s+");
        String[] cleanedWords = new String[originalWords.length];
        
        for (int i = 0; i < originalWords.length; i++) {
            String cleaned = originalWords[i].replaceAll("[^a-zA-Z]", "").toLowerCase();
            wordMapping.put(cleaned, originalWords[i]);
            cleanedWords[i] = cleaned;
        }
        
        StringBuilder poem = new StringBuilder();
        
        for (int i = 0; i < cleanedWords.length; i++) {
            String originalWord = wordMapping.getOrDefault(cleanedWords[i], "");
            poem.append(originalWord); // Append original word with punctuation
            
            if (i < cleanedWords.length - 1) {
                String word1 = cleanedWords[i];
                String word2 = cleanedWords[i + 1];
                
                String bridgeWord = findBridgeWord(word1, word2);
                
                if (bridgeWord != null) {
                    poem.append(" ").append(wordMapping.getOrDefault(bridgeWord, bridgeWord));
                }
            }
